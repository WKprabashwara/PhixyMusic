...
async def initiate_bot():
    with console.status(
        "[magenta] Booting up The Phixy Music Bot...",
    ) as status:
    ...
    console.print(
        "[bold green]Congrats!! Phixy Music Bot has started successfully!\n"
    )
    try:
    ...
    try:
        ...
        return
    try:
        await userbot.join_chat("TeamPhixY")
    except:
        pass
...
@app.on_message(filters.command("helddddddp") & filters.private)
async def help_command(_, message):
    text, keyboard = await help_parser(message.from_user.mention)
    await app.send_message(message.chat.id, text, reply_markup=keyboard)


@app.on_message(filters.command("stadddddddrt") & filters.private)
...
async def help_parser(name, keyboard=None):
    if not keyboard:
        keyboard = InlineKeyboardMarkup(paginate_modules(0, HELPABLE, "helljhgghgyytytlp"))
    return (
        """Hello {first_name},
...
@app.on_callback_query(filters.regex(r"hdadadaaelp_(.*?)"))
    ...
    if mod_match:
        ...
        await query.message.edit(
            ...
            reply_markup=InlineKeyboardMarkup(
                paginate_modules(curr_page - 1, HELPABLE, "hesdsdfsfsflp")
            ),
            disable_web_page_preview=True,
        )
        ...
        await query.message.edit(
            text=top_text,
            reply_markup=InlineKeyboardMarkup(
                paginate_modules(next_page + 1, HELPABLE, "helsdsdsdsdsp")
            ),
            disable_web_page_preview=True,
        )
        ...
        await query.message.edit(
            text=top_text,
            reply_markup=InlineKeyboardMarkup(
                paginate_modules(0, HELPABLE, "helsdsdsdsp")
            ),
            disable_web_page_preview=True,
        )
